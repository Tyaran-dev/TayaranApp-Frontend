...
const useBookingHandler = (
  ...
  const handleSubmitBooking = useCallback(async () => {
    ...
    const submittedAdults = bookingData.CustomerDetails.reduce(
      (sum, room) =>
        sum + room.CustomerNames.filter((name) => name.Type === "Adult").length,
      0
    );
    const submittedChildren = bookingData.CustomerDetails.reduce(
      (sum, room) =>
        sum + room.CustomerNames.filter((name) => name.Type === "Child").length,
      0
    );
    ...
    if (
      ...
      throw new Error(`Please enter information for all guests. 
